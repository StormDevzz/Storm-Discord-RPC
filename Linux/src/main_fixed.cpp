...
public:
    ...
    void on_activate() override {
        ...
        auto connect_window = Gtk::make_managed<ConnectWindow>();
        connect_window->signal_hide().connect(sigc::mem_fun(*this, &DiscordApp::on_connect_window_hidden));
...
private:
    ConnectWindow* current_connect_window = nullptr;
    MainWindow* current_main_window = nullptr;
    std::string client_id;

    // обработчик закрытия окна подключения
    void on_connect_window_hidden() {
        if (current_connect_window && current_connect_window->is_connected()) {
            client_id = current_connect_window->get_client_id();
            
            // создаем главное окно
            current_main_window = Gtk::make_managed<MainWindow>(client_id);
            current_main_window->signal_hide().connect(sigc::mem_fun(*this, &DiscordApp::on_main_window_hidden));
            
            add_window(*current_main_window);
            current_main_window->show();
        }
        
        if (current_connect_window) {
            remove_window(*current_connect_window);
            current_connect_window = nullptr;
        }
    }

    // обработчик закрытия главного окна
    void on_main_window_hidden() {
        if (current_main_window) {
            remove_window(*current_main_window);
            current_main_window = nullptr;
        }
    }
};
